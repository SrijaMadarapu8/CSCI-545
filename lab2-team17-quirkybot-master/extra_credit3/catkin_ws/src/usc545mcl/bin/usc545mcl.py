...
# @numba.jit(nopython=True)
def cluster_particles(particles, epsilon, min_samples):
    particle_positions = np.array([(particle.map_T_particle.translation[0], particle.map_T_particle.translation[1], particle.map_T_particle.rotation) for particle in particles])
    print(particle_positions.shape)
    # Use DBSCAN for clustering particles
    db = DBSCAN(eps=epsilon, min_samples=min_samples).fit(particle_positions)
    
    # Get the labels assigned to each particle by DBSCAN
    labels = db.labels_
    
    # Find the label that represents the largest mode
    unique_labels, label_counts = np.unique(labels, return_counts=True)
    largest_mode_label = unique_labels[np.argmax(label_counts)]
    
    # Get the indices of particles belonging to the largest mode
    largest_mode_indices = np.where(labels == largest_mode_label)[0]
    largest_mode_particles = [particles[i] for i in largest_mode_indices]
    
    return largest_mode_particles
...
class Particle(object):
    ...
    def UpdateOdom(self, odom_msg):
        ...
        # Decompose the motion into delta_rot1, delta_trans, and delta_rot2
        vx = odom_msg.twist.twist.linear.x
        vy = odom_msg.twist.twist.linear.y
        local_velocity = np.array([vx, vy], dtype=dtype)
        world_velocity = RotateBy(local_velocity, self.map_T_particle.rotation)
        xt = self.map_T_particle.translation + world_velocity * dt  # x_t
        xtm1 = self.map_T_particle.translation  # x_(t-1)
        dx = xt - xtm1
        omega = odom_msg.twist.twist.angular.z
        rotation = self.map_T_particle.rotation + omega * dt  # theta_t

        # Calculate the decomposed components: delta_rot1, delta_trans, and delta_rot2
        delta_rot1 = np.arctan2(dx[1], dx[0]) - self.map_T_particle.rotation
        delta_trans = np.sqrt(dx[0]**2 + dx[1]**2)
        delta_rot2 = rotation - self.map_T_particle.rotation - delta_rot1

        # Sample from motion model using noise parameters a1, a2, a3, a4
        a1, a2, a3, a4 = NOISE_PARAMS
        delta_hat_rot1 = delta_rot1 - np.random.normal(0, a1 * np.abs(delta_rot1) + a2 * np.abs(delta_trans))
        delta_hat_rot2 = delta_rot2 - np.random.normal(0, a1 * np.abs(delta_rot2) + a2 * np.abs(delta_trans))
        delta_hat_trans = delta_trans - np.random.normal(0, a3 * np.abs(delta_trans) + a4 * (np.abs(delta_rot1) + np.abs(delta_rot2)))

        # Update the particle's pose using the sampled values
        delta_x = delta_hat_trans * np.array([np.cos(self.map_T_particle.rotation + delta_hat_rot1),
                                              np.sin(self.map_T_particle.rotation + delta_hat_rot1)])
        self.map_T_particle.translation += delta_x
        self.map_T_particle.rotation += (delta_hat_rot1 + delta_hat_rot2)
    ...
    def UpdateScan(self, scan):
        ...
        for i in range(len(scan.angles)):
            ...
            simulated_range = sim_ranges[i]

            # Calculate the Gaussian probability density function
            # You can use numpy's Gaussian PDF function for this
            probability = np.exp(-0.5 * 
                (true_range - simulated_range)**2 / SENSOR_MODEL_VAR) / (
                    np.sqrt(2 * np.pi * SENSOR_MODEL_VAR))
        ...
        self.weight = np.prod(likelihoods)
        #
        ##########
...
class ParticleFilter(object):
    ...
    def GetMeanPose(self):
        ...
        sum_rotation = 0
        min_samples, epsilon = MIN_SAMPLES, EPSILON  
        largest_mode_particles = cluster_particles(self.particles, epsilon, min_samples)
        for particle in largest_mode_particles:
    ...
    def UpdateScan(self, scan_msg):
        ...
        normalized_prob = np.array(weights, dtype=dtype) / sum(weights)
        ...
        selected_indices = np.random.choice(num_particles, size=num_particles, p=normalized_prob)
