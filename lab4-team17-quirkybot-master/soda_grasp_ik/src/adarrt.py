...
class AdaRRT():
    ...
    def build(self):
        ...
        for k in range(self.max_iter):
            ...
            if rand_num < 2:
                rand_point = self._get_random_sample_near_goal()
            else:
                rand_point = self._get_random_sample()

            nearest_neighbor = self._get_nearest_neighbor(rand_point)
            new_node = self._extend_sample(rand_point, nearest_neighbor)
    ...
    def _get_random_sample(self):
        ...
        """
        # FILL in your code here
        rand_point = np.asarray(list(map(np.random.uniform, self.joint_lower_limits, self.joint_upper_limits)))
        return rand_point
    ...
    def _get_nearest_neighbor(self, sample):
        ...
        """
        # FILL in your code here
        min_dist, nearest_neighbor = np.inf, None
    ...
    def _get_random_sample_near_goal(self):
        rand_point = np.asarray(list(map(np.random.uniform, self.goal.state - 0.05, self.goal.state + 0.05)))
        return rand_point
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        new_point = neighbor.state + self.step_size * direction
    ...
    def _check_for_completion(self, node):
        ...
        return dist <= self.goal_precision
