...
class AdaRRT():
    ...
    def build(self):
        ...
        for k in range(self.max_iter):
            ...
            if probability < 2:
                samplePoint = self._get_random_sample_near_goal()
            else:
                samplePoint = self._get_random_sample()
            samplePoint = np.asarray(samplePoint)
            nearestNeighbor = self._get_nearest_neighbor(samplePoint)
            new_node = self._extend_sample(samplePoint, nearestNeighbor)
    ...
    def _get_random_sample(self):
        ...
        """
        # FILL in your code here
        dim_min = np.asarray(self.joint_lower_limits)
        dim_max = np.asarray(self.joint_upper_limits)
        sample = np.random.uniform(low=dim_min, high=dim_max, size=(1,6))
        return sample[0]

    def _get_random_sample_near_goal(self):
        # FILL in your code here
        dim_min = np.asarray(self.goal.state - [0.05]*6)
        dim_max = np.asarray(self.goal.state + [0.05]*6)
        sample = np.random.uniform(low=dim_min, high=dim_max, size=(1,6))
        return sample[0]
    ...
    def _get_nearest_neighbor(self, sample):
        ...
        """
        # FILL in your code here
        minObject = [np.inf, AdaRRT.Node([], None)]
        for node in self.start.__iter__():
            if np.sqrt(np.sum(np.square(np.asarray(sample) - np.asarray(node.state)))) < minObject[0]:
                minObject[0] = np.sqrt(np.sum(np.square(np.asarray(sample) - np.asarray(node.state))))
                minObject[1] = node
        return minObject[1]
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        neighborState = np.asarray(neighbor.state)
        direction = (sample - neighborState) / np.sqrt(np.sum(np.square(sample - neighborState)))
        newState = self.step_size * direction + neighborState
        if self._check_for_collision(newState):
            return 
        else:
            newNode = neighbor.add_child(newState)
            return newNode
    ...
    def _check_for_completion(self, node):
        ...
        """
        # FILL in your code here
        if np.sqrt(np.sum(np.square(self.goal.state - node.state))) <= self.goal_precision:
            return True
        return False
